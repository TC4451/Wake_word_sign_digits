...
#normailize values to be between -1,1
Audio_Amp_Data = Audio_Amp_Data /np.max(np.abs(Audio_Amp_Data))

print(f"Audio_Amp_Data : {Audio_Amp_Data}")
print(len(Audio_Amp_Data))
print(f"Sampling rate : {Sample_Rate}")

plt.figure(figsize=(10, 4))
plt.plot(Audio_Amp_Data)
plt.show()

# Get the Mel-frequency cepstral coefficient
...
def convert_wav_to_mcff(audio_file):
    #print(audio_file) - I used this to debug a bad wav file
    Audio_Amp_Data, Sample_Rate = librosa.load(audio_file)
    Audio_Amp_Data = Audio_Amp_Data / np.max(np.abs(Audio_Amp_Data))
